...
5. **Eliminar borrador** (`DELETE /sales/:id`): solo `DRAFT`. Borra el documento en espera y sus líneas (no hay stock que revertir). En el POS: botón **Cargar factura** → papelera en cada ítem. Una factura confirmada no se elimina por esta vía.
