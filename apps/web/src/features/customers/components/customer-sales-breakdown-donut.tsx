...
type CustomerSalesBreakdownDonutProps = {
  totalUsd: number
  pagadoUsd: number
  creditoUsd: number
  size?: number
...
export function CustomerSalesBreakdownDonut({
  totalUsd,
  pagadoUsd,
  creditoUsd,
  size = 140,
  compact = false,
  className,
}: CustomerSalesBreakdownDonutProps) {
  ...
  return (
    ...
    >
      <div
        ...
        role="img"
        aria-label={`Total facturado ${totalUsd.toFixed(2)} USD. Abonado ${pagadoUsd.toFixed(2)}. Crédito pendiente ${creditoUsd.toFixed(2)}.`}
      >
      ...
      <div className={cn('flex w-full flex-col gap-2', compact ? 'text-[10px]' : 'text-xs')}>
        <div className="flex items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-1.5 text-white/70">
            ...
            <span className="truncate">Abonado</span>
          </div>
          <DisplayMoneyFromUsd
            amountUsd={pagadoUsd}
