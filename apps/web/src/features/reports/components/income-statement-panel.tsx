...
import { IncomeStatementKpiGrid } from '@/features/reports/components/income-statement-kpi-grid'
...
import { useIncomeStatementQuery } from '@/features/reports/hooks/use-reports'
import {
...
function defaultIncomeStatementFilters(): IncomeStatementFilters {
...
export function IncomeStatementPanel() {
  ...
  const [filters, setFilters] = useSessionPersistedState(
    sessionFilterKey('reports-income-statement', company?.id),
    defaultIncomeStatementFilters()
  )
  ...
  const { data, isLoading, isError, error } = useIncomeStatementQuery(queryParams)
    ...
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <IncomeStatementKpiGrid summary={data.summary} />
        </>
      )}
    </div>
  )
