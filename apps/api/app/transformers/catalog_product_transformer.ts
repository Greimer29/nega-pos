...
export function serializeCatalogProduct(
  ...
  const sizes = product.$preloaded.sizes
    ? [...product.sizes].sort((a, b) => a.size.localeCompare(b.size, undefined, { numeric: true }))
    : []
  ...
  return {
    ...
    wholesale_units_per_pack: product.wholesaleUnitsPerPack,
    wholesale_cost_usd: product.wholesaleCostUsd,
    wholesale_sale_price_usd: product.wholesaleSalePriceUsd,
